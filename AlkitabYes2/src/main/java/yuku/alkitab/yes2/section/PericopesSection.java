...
public class PericopesSection extends SectionContent implements SectionContent.Writer {
    ...
    public void write(RandomOutputStream output) throws IOException {
        ...
        for (int i = 0; i < entry_count; i++) {
            ...
            // if the difference from the previous ari is <= 0x7fff, it's written in 2bytes: (0x8000 | difference)
            // otherwise it's written in 4 bytes: (0x00ffffff & ari)
            int diff_ari = entry.ari - last_entry_ari;
            if (last_entry_ari == 0 || (diff_ari < 0 || diff_ari > 0x7fff)) {
                bw.writeInt((0x00ffffff & entry.ari));
            } else {
