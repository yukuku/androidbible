...
public class ConvertPdbToYes2 {
	...
	SparseIntArray bookIdToPdbBookPosMap_;
	...
	public ConvertResult convert(final Context context, String filenamepdb, File yesFile, ConvertParams params) {
		...
		try {
			...
			progress(30, context.getString(R.string.cp_analyzing_available_books));
			{
				...
				bookIdToPdbBookPosMap_ = new SparseIntArray();
			}
			...
			for (int bookPos = 0; bookPos < nbook; bookPos++) {
				...
				if (bookId < 0) {
					...
					if (bookIdToPdbBookPosMap_.indexOfKey(bookId) >= 0) {
			...
			final int[] sortedBookIds = new int[bookIdToPdbBookPosMap_.size()];
			for (int i = 0, len = sortedBookIds.length; i < len; i++) {
				sortedBookIds[i] = bookIdToPdbBookPosMap_.keyAt(i);
			}
