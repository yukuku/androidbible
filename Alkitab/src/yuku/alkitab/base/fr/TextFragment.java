...
public class TextFragment extends BaseFragment implements XrefDialog.XrefDialogListener {
	...
	Search2Engine.Query search2_query = null;
	...
	public void onAttach(final Activity activity) {
		...
		this.activity = (IsiActivity) activity;
	}
	...
	public View onCreateView(final LayoutInflater inflater, final ViewGroup container, final Bundle savedInstanceState) {
		...
		final View res = inflater.inflate(R.layout.fragment_text, container, false);

		overlayContainer = V.get(res, R.id.overlayContainer);
		root = V.get(res, R.id.root);
		lsText = V.get(res, R.id.lsSplit0);
		lsSplit1 = V.get(res, R.id.lsSplit1);
		tSplitEmpty = V.get(res, R.id.tSplitEmpty);
		splitRoot = V.get(res, R.id.splitRoot);
		splitHandle = V.get(res, R.id.splitHandle);
		splitHandleButton = V.get(res, R.id.splitHandleButton);
		panelNavigation = V.get(res, R.id.panelNavigation);
		bGoto = V.get(res, R.id.bGoto);
		bLeft = V.get(res, R.id.bLeft);
		bRight = V.get(res, R.id.bRight);
	...
	int jumpTo(String reference) {
		...
		if (! jumper.getParseSucceeded()) {
			Toast.makeText(activity, getString(R.string.alamat_tidak_sah_alamat, reference), Toast.LENGTH_SHORT).show();
			return 0;
		}
	...
	public void onStop() {
		...
		SharedPreferences.Editor editor = instant_pref.edit();
	...
	void bGoto_longClick() {
		if (history.getSize() > 0) {
			...
			Toast.makeText(activity, R.string.belum_ada_sejarah, Toast.LENGTH_SHORT).show();
		}
	}
	...
	@Override public void onPrepareOptionsMenu(Menu menu) {
		...
		menu.findItem(R.id.menuFullScreen).setChecked(activity.isFullScreen());
	}
	...
	void setShowTextAppearancePanel(boolean yes) {
		if (yes) {
			if (textAppearancePanel == null) { // not showing yet
				textAppearancePanel = new TextAppearancePanel(activity, inflater, overlayContainer, new TextAppearancePanel.Listener() {
	...
	void openVersionsDialog() {
		Pair<List<String>, List<VersionsActivity.MVersion>> versions = getAvailableVersions();
	...
	void openSplitVersionsDialog() {
		...
		.setSingleChoiceItems(options.toArray(new String[options.size()]), selected, new DialogInterface.OnClickListener() {
			...
			void openSplit() {
				...
				splitHandle.measure(View.MeasureSpec.makeMeasureSpec(lsText.getWidth(), View.MeasureSpec.AT_MOST), View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED));
...
//			}
	...
	VersesView.AttributeListener attributeListener = new VersesView.AttributeListener() {
		public void onAttributeClick(Book book, int chapter_1, int verse_1, int kind) {
			if (kind == Db.Bookmark2.kind_bookmark) {
				...
				TypeBookmarkDialog dialog = new TypeBookmarkDialog(activity, reference, ari);
				...
				TypeNoteDialog dialog = new TypeNoteDialog(activity, book, chapter_1, verse_1, new TypeNoteDialog.Listener() {
	...
	ActionMode.Callback actionMode_callback = new ActionMode.Callback() {
		@Override public boolean onCreateActionMode(ActionMode mode, Menu menu) {
			...
			if (hasEsvsbAsal == null) {
				try {
					App.context.getPackageManager().getApplicationInfo("yuku.esvsbasal", 0); //$NON-NLS-1$
					hasEsvsbAsal = true;
		...
		@Override public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
			...
			switch (itemId) {
				...
				case R.id.menuShare: {
					...
					String verseUrl;
					if (selected.size() == 1) {
						verseUrl = createVerseUrl(TextFragment.this.activeBook, TextFragment.this.chapter_1, String.valueOf(selected.get(0)));
					} else {
						StringBuilder sb = new StringBuilder();
						Book.writeVerseRange(selected, sb);
						verseUrl = createVerseUrl(TextFragment.this.activeBook, TextFragment.this.chapter_1, sb.toString()); // use verse range
					}
				...
				case R.id.menuAddBookmark: {
					...
					final int ari = Ari.encode(TextFragment.this.activeBook.bookId, TextFragment.this.chapter_1, mainVerse_1);

					TypeBookmarkDialog dialog = new TypeBookmarkDialog(activity, TextFragment.this.activeBook.reference(TextFragment.this.chapter_1, mainVerse_1), ari);
					dialog.setListener(new TypeBookmarkDialog.Listener() {
						@Override public void onOk() {
							lsText.uncheckAllVerses(true);
							lsText.loadAttributeMap();
				...
				case R.id.menuAddNote: {
					...
					TypeNoteDialog dialog = new TypeNoteDialog(activity, TextFragment.this.activeBook, TextFragment.this.chapter_1, mainVerse_1, new TypeNoteDialog.Listener() {
						@Override public void onDone() {
							lsText.uncheckAllVerses(true);
							lsText.loadAttributeMap();
				...
				case R.id.menuAddHighlight: {
					final int ariKp = Ari.encode(TextFragment.this.activeBook.bookId, TextFragment.this.chapter_1, 0);
					int colorRgb = S.getDb().getHighlightColorRgb(ariKp, selected);

					new TypeHighlightDialog(activity, ariKp, selected, new TypeHighlightDialog.Listener() {
	...
	public void handleViewIntent(final int ari) {
		jumpToAri(ari);
		history.add(ari);
	}

	public int getCurrentDisplayedAri() {
		return Ari.encode(this.activeBook.bookId, this.chapter_1, lsText.getVerseBasedOnScroll());
	}
}
