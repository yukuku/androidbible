...
4. **Receivers (Java custom views)** — `LabeledSplitHandleButton.onAttachedToWindow` and `LeftDrawer.Text.onFinishInflate` now call a third helper, `AppEvents.observeOnView(view, flow, runnable)`, which launches a `CoroutineScope(Dispatchers.Main + SupervisorJob())` and cancels it on `onViewDetachedFromWindow`. Matches the legacy "single observation window bounded by detach" behavior.
