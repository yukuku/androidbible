...
- `Modifier.rubyOverlay` paints each reading over its base using the `TextLayoutResult`, centred when it fits and slid towards the side with room when it is wider (`rubyLeftPx`); a base run broken across lines gets a proportional slice of the reading on each line (`rubySliceFor`). The reading takes the color of the innermost colored span under its centre character (`rubyColorAt`), so red-letter and highlighted, selected runs stay readable.
