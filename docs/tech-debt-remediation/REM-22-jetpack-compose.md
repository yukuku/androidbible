...
- `BibleAppTheme` introduced under `yuku.alkitab.base.compose`. Since 2026-08-09 it applies `BibleAppDarkColorScheme`, a static dark Material 3 scheme built from the app's XML palette (`accent`/`escape`/`primary`/`window` color resources) instead of wallpaper-derived dynamic colors, so Compose surfaces show the same brand colors on every device as the View screens. `BibleAppThemeColorsTest` pins the scheme roles to the XML color resources, and `ComposeThemeSnapshotTest` renders the real Compose surfaces into a side-by-side HTML report (dynamic vs branded) under `Alkitab/build/snapshots/compose-theme/`.
