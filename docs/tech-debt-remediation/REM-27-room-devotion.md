...
**Difficulty:** Easy. Took ~half a day with the REM-10 / REM-11 pattern already established. `PerVersion` followed as [REM-28](REM-28-room-per-version.md), `ProgressMark` / `ProgressMarkHistory` as [REM-29](REM-29-room-progress-mark.md), `ReadingPlan` / `ReadingPlanProgress` as [REM-30](REM-30-room-reading-plan.md); remaining tables (`SyncShadow`, `SyncLog`) can follow the same pattern.
