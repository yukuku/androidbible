...
The renderer is chosen by the `ExperimentalFlags.useComposeSong()` flag, exposed under **Settings → Experimental features → "Song lyrics (legacy views)"** (`pref_useLegacySong_key`, unchecked by default, so Compose renders). Ticking it falls back to the WebView, the same pattern as the other legacy-view opt-outs. Open a song after toggling to see the change.
