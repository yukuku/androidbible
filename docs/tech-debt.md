...
| `AmbilWarna` | bundled | ⚠️ **Partially fixed in REM-20** (2026-05-11). The two `AmbilWarnaDialog` call sites (`MarkersActivity`, `TypeHighlightDialog`) now use a new iOS-style Compose color picker (`IosColorPicker` + `ColorPickerDialog` wrapper). The `AmbilWarna` module is still on the classpath because 9 `AmbilWarnaPreference` widget entries in `color_settings.xml`, `color_settings_night.xml`, and `settings_display.xml` continue to depend on it — those need a `Preference`-subclass replacement before the module can be deleted. |
